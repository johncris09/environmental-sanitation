...
const Login = () => {
  ...
  useEffect(() => {
    const token = localStorage.getItem('enviromentalSanitationToken') // Assuming the token is stored in local storage
    if (token) {
      // If the token is set, navigate to the equipment
      navigate('/sanitary_permit', { replace: true })
    }
  }, [navigate])
        ...
        await api
          ...
          .then((response) => {
            if (response.data.status) {
              ...
              localStorage.setItem('enviromentalSanitationToken', response.data.token)
              ...
              <CCard className="p-4" style={{ position: 'relative' }}>
                <CCardBody>
                  ...
                  <CForm
                    ...
                    validated={validated}
                  >
                    <h3 className="text-center ">Enviromental Sanitation</h3>
